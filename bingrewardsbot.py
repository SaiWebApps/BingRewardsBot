...
class BingRewardsBotManager:
    ...
    def execute(self, desktop_accounts = [], mobile_accounts = []):
        bots = [DesktopBingRewardsBot(self.desktop_bot_config, account) for account in desktop_accounts]
        bots.extend([MobileBingRewardsBot(self.mobile_bot_config, account) for account in mobile_accounts])
        for bot in bots:
            bot.start()